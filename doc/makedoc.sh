...
echo "QHP_NAMESPACE = \"com.skycoder42.qtdatasync.$verTag\"" >> $doxyRes
echo "QHP_CUST_FILTER_NAME = \"DataSync $version\"" >> $doxyRes
echo "QHP_CUST_FILTER_ATTRS = \"qtdatasync $version\"" >> $doxyRes
echo "QHG_LOCATION = \"$qtBins/qhelpgenerator\"" >> $doxyRes
echo "INCLUDE_PATH += \"$qtHeaders\"" >> $doxyRes
echo "GENERATE_TAGFILE = \"$destDir/qtdatasync/qtdatasync.tags\"" >> $doxyRes
if [ "$DOXY_STYLE" ]; then
